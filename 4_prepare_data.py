...
def return_trajectory_chunk(start, end):
    ...
    traj_df =pd.read_csv(root+'output_data/trajectory{}.csv'.format(year), parse_dates=['datetime_ut']) 
...
df=pd.read_csv(root+"ML_Dataset/ML_total_timestamps.csv",parse_dates=['start','end'],index_col=False)
df=df.loc[:,['start','end','label']]
df_aug1=pd.read_csv(root+"ML_Dataset/ML_lfeaug1_timestamps.csv",parse_dates=['start','end'],index_col=False)
df_aug1=df_aug1.loc[:,['start','end','label']]
df_aug2=pd.read_csv(root+"ML_Dataset/ML_lfeaug2_timestamps.csv",parse_dates=['start','end'],index_col=False)
df_aug2=df_aug2.loc[:,['start','end','label']]
df_aug3=pd.read_csv(root+"ML_Dataset/ML_lfeaug3_timestamps.csv",parse_dates=['start','end'],index_col=False).loc[:,['start','end']]
df_aug3['label']=np.repeat('LFE_aug3',len(df_aug3))
df_aug4=pd.read_csv(root+"ML_Dataset/ML_lfeaug4_timestamps.csv",parse_dates=['start','end'],index_col=False).loc[:,['start','end']]
df_aug4['label']=np.repeat('LFE_aug4',len(df_aug4))
df_aug5=pd.read_csv(root+"ML_Dataset/ML_lfeaug5_timestamps.csv",parse_dates=['start','end'],index_col=False).loc[:,['start','end']]
df_aug5['label']=np.repeat('LFE_aug5',len(df_aug5))
total_df=pd.concat([df, df_aug1, df_aug2,df_aug3, df_aug4, df_aug5],axis=0)
total_df.to_csv(root+"ML_Dataset/ML_total_timestamps_withaug.csv",index=False)
...
total_df.to_csv(root+"ML_Dataset/ML_total_catalogue_withaug.csv",index=False)
