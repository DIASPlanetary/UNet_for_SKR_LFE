...
df=pd.read_csv(root+'ML_Dataset/ML_lfe_timestamps.csv',parse_dates=['start','end'])
nolfe_df = pd.read_csv(root+'ML_Dataset/ML_nolfe_timestamps.csv',parse_dates=['start','end'])

random.seed=42
len_aug1 = 50
sample_lfe=random.sample(list(np.arange(len(df))), len_aug1)
...
lfe_df_augment1 = df.loc[sample_lfe,:].reset_index(drop=True)
lfe_df_augment1['label']=np.repeat('LFE_aug1', len(lfe_df_augment1))
lfe_df_augment1.to_csv(root+"ML_Dataset/ML_lfeaug1_timestamps.csv")
...
for i, j, k, l in zip(s_lfe, e_lfe, s_nolfe, e_nolfe):
    ...
    if year_lfe == '2017':
        file_lfe = root+'input_data/SKR_2017_001-258_CJ.sav'
    else: 
        file_lfe = root+'input_data/SKR_{}_CJ.sav'.format(year_lfe)
        
    if year_nolfe == '2017':
        file_nolfe = root+'input_data/SKR_2017_001-258_CJ.sav'
    else: 
        file_nolfe = root+'input_data/SKR_{}_CJ.sav'.format(year_nolfe)
    plt.ioff()
    fp_sav = root+"ML_Dataset"
    polygon_fp=root+"output_data/ML_lfes.json"
    
    a = plot_andaugment_pol_and_flux1(i,j, k,l, file_lfe, file_nolfe, count, fp_sav,side='right')
    b=find_mask_aug(time_view_start=i, time_view_end=j, val='s', file_data=file_lfe,file_nolfe=file_nolfe,\
                    polygon_fp=polygon_fp,ind=count,fp_sav=fp_sav,augment_1=True,\
...
len_aug2 = 50
random.seed=10
sample_lfe=random.sample(list(np.arange(len(df))), len_aug2)
...
lfe_df_augment2 = df.loc[sample_lfe,:].reset_index(drop=True)
lfe_df_augment2['label']=np.repeat('LFE_aug2', len(lfe_df_augment2))
lfe_df_augment2.to_csv(root+"ML_Dataset/ML_lfeaug2_timestamps.csv")
...
for i, j, k, l in zip(s_lfe, e_lfe, s_nolfe, e_nolfe):
    ...
    a=plot_andaugment_pol_and_flux1(time_view_start=i, time_view_end=j, nolfe_start=k,nolfe_end=l, 
                                  file=file_lfe,file2=file_nolfe,ind=count,fp_sav=fp_sav,side='left')
    b=find_mask_aug(time_view_start=i, time_view_end=j, val='s', file_data=file_lfe,file_nolfe=file_nolfe,\
                    polygon_fp=polygon_fp,ind=count,fp_sav=fp_sav,augment_1=False,\
...
'''____Type 3___'''
dur=df.apply(lambda x: (x.end-x.start).total_seconds()/3600,axis=1)
df['dur']=dur

len_aug3and4=100
test_df1=df.loc[(df['dur']>8)&(df['dur']<12)&(df['label']!='LFE_sm'),:].reset_index(drop=True)
...
picked_shifts=[]
count=len(total_df) + len_aug1 +len_aug2
...
for i,j in zip(lfe_df_augment3['start'], lfe_df_augment3['end']):
    ...
    a = plot_andaugment_pol_and_flux2(i,j, file_lfe, fp_sav,count,shift,side='left')
    b=find_mask_aug(time_view_start=i, time_view_end=j, val='s', file_data=file_lfe,file_nolfe=None,\
                    polygon_fp=polygon_fp,ind=count,fp_sav=fp_sav,augment_1=False,\
                    augment_2=False,augment_3=True,time_aug1=None, time_aug2=None,shift=shift,side='left')
    print(count)
    count+=1
lfe_df_augment3['shift']=picked_shifts
lfe_df_augment3.to_csv(root+"ML_Dataset/ML_lfeaug3_timestamps.csv",index=False)


''' Type 4'''
len_aug4 = split
lfe_df_augment4=test_df1.loc[split:,:].reset_index(drop=True)
count=len(total_df) + len_aug1 +len_aug2 +len_aug3
picked_shifts=[]
for i,j in zip(lfe_df_augment4['start'], lfe_df_augment4['end']):
    ...
    if year_lfe == '2017':
        ...
        file_lfe = root+'input_data/SKR_{}_CJ.sav'.format(year_lfe)
    fp_sav = "C:/Users/eliza/Desktop/git_folder/ML_For_SKR_Code/ML_Dataset/Main_Dataset"
    polygon_fp=root+"output_data/ML_lfes.json"
    shift=random.choice(shifts)
    picked_shifts.append(shift)
    a = plot_andaugment_pol_and_flux2(i,j, file_lfe, fp_sav,count,shift,side='right')
    b=find_mask_aug(time_view_start=i, time_view_end=j, val='s', file_data=file_lfe,file_nolfe=None,\
                    polygon_fp=polygon_fp,ind=count,fp_sav=fp_sav,augment_1=False,\
...
lfe_df_augment4.to_csv(root+"ML_Dataset/ML_lfeaug4_timestamps.csv",index=False)


'''____Type 4___Multiple LFEs in one image'''
lfe_df_06 = df.loc[df['start'].between(pd.Timestamp('20060101'), pd.Timestamp('20070101')),:].reset_index(drop=True)

lfe_df_dec = df.loc[df['start'].between(pd.Timestamp('20081201'), pd.Timestamp('20090101')),:].reset_index(drop=True)
lfe_df_mar= df.loc[df['start'].between(pd.Timestamp('20080301'), pd.Timestamp('20080401')),:].reset_index(drop=True)
lfe_df_may= df.loc[df['start'].between(pd.Timestamp('20080501'), pd.Timestamp('20080601')),:].reset_index(drop=True)
...
lfe_df_augment5.to_csv(root+"ML_Dataset/ML_lfeaug5_timestamps.csv",index=False)

count=len(total_df) + len_aug1 +len_aug2 +len_aug3+len_aug4
for i,j in zip(starts,ends):
    ...
    fp_sav = root+"ML_Dataset"
    polygon_fp=rot+"output_data/ML_lfes.json"
    plt.ioff()
    a = plot_pol_and_flux(i, j, file_lfe, count, fp_sav)
    b=find_mask(i, j, 's', file_lfe, polygon_fp, count, fp_sav)
    print(count)
    count+=1
